...
You may also launch the initialization from the batch file `run_init.sh` and then the simulation on the same partition `run_sim.sh` modifying them according to your cluster/laptop settings.

For compatibility reasons, is best to use the version of PETSc <=3.19.6 